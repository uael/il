#ifndef _JAYL_BACKEND_FUNCTION_H
#define _JAYL_BACKEND_FUNCTION_H
...
namespace jayl {
  ...
  namespace backend {
    ...
    class Function : public interfaces::Printable, interfaces::Uncopyable {
      ...
      virtual void bind(const std::string &name, jayl::Set *set) = 0;
      ...
      /// We store the Jayl Function's literals to prevent their memory from being
      /// reclaimed if the IR is deleted, as compiled functions are allowed to
      /// access them at runtime.
      std::vector<jayl::ir::Expr> literals;
    };
