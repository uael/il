...
static struct a_firm_opt firm_opt = {
  .const_folding    =  true, .cse              =  true, .confirm          =  true, .muls             =  true, .divs             =  true, .mods             =  true, .strict_alias     =  false, .no_alias         =  false,
#ifdef NO_DEFAULT_VERIFY
  .verify           =  false,
#else
  .verify           =  true,
#endif
  .check_all        =  true, .clone_threshold  =  DEFAULT_CLONE_THRESHOLD, .inline_maxsize   =  750, .inline_threshold =  0,
};

/* dumping options */
static struct a_firm_dump firm_dump = {
  .debug_print  = false, .all_types    = false, .ir_graph     = false, .all_phases   = false,
};
...
typedef enum opt_flags {
  ...
  OPT_FLAG_HIDE_OPTIONS = 1 << 3, /**< do not automatically process -foptions for this transformation */
  OPT_FLAG_ESSENTIAL = 1 << 4, /**< output won't work without this pass so we need it even with -O0 */
} opt_flags_t;
...
#define IRG(a, b, c, d) { OPT_TARGET_IRG, a, .u.transform_irg = b, .description = c, .flags = d }
#define IRP(a, b, c, d) { OPT_TARGET_IRP, a, .u.transform_irp = b, .description = c, .flags = d }
...
#define FOR_EACH_OPT(i) for (opt_config_t *i = opts; i != ((opts) + ARRAY_SIZE(opts)); ++i)
...
static void do_firm_optimizations(void) {
  ...
  set_irp_memory_disambiguator_options((ir_disambiguator_options) aa_opt);
...
void set_be_option(char const *const arg) {
  ...
  if (!res)
    error("setting firm backend option '%s' failed (maybe an outdated version of firm is used)", arg);
}
