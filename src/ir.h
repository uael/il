...
namespace jayl {
   namespace ir {
    /**
     * The base class of all nodes in the Jayl Intermediate Representation
     * (Jayl IR)
     */
    struct IRNode : private jayl::interfaces::Uncopyable {
      public:
      IRNode() {}
    ...
    class Expr : public IRHandle {
      ...
      template<typename E>
      friend bool isa(Expr);

      template<typename E>
      friend const E *to(Expr);
    };
    ...
    class Stmt : public IRHandle {
      ...
      template<typename S>
      friend bool isa(Stmt);

      template<typename S>
      friend const S *to(Stmt);
    };
...
///     have more than one result.
    ...
    struct ForDomain {
      enum Kind {
        IndexSet, Endpoints, Edges, Neighbors, NeighborsOf,
        Diagonal, Grid
      };
      Kind kind;
