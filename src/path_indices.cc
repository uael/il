...
// class SetEndpointPathIndex
    SetEndpointPathIndex::SetEndpointPathIndex(const jayl::Set &edgeSet)
      : edgeSet(edgeSet) {
    ...
    SetEndpointPathIndex::neighbors(unsigned elemID) const {
      class SetEndpointNeighbors : public PathIndexImpl::Neighbors::Base {
        class Iterator : public PathIndexImpl::Neighbors::Iterator::Base {
          public:
          Iterator(const jayl::Set::Endpoints::Iterator &epit) : epit(epit) {}
          ...
          private:
          jayl::Set::Endpoints::Iterator epit;
        };

        public:
        SetEndpointNeighbors(const jayl::Set::Endpoints &endpoints)
          : endpoints(endpoints) {}
...
// class PathIndexBuilder
        ...
        void visit(const Link *link) {
          switch (link->getType()) {
            ...
            case Link::vv: {
              ...
              const jayl::Set &throughSet =
                *builder->getBinding(stencil.getGridSet());

              map<unsigned, vector<unsigned>> pathNeighbors;

              // create neighbor lists
              const jayl::Set &sourceSet =
    ...
    void PathIndexBuilder::bind(std::string name, const jayl::Set *set) {
      bindings.insert({name, set});
    }

    const jayl::Set *PathIndexBuilder::getBinding(pe::Set pset) const {
      iassert(pset.defined());
      return bindings.at(pset.getName());
    }

    const jayl::Set *PathIndexBuilder::getBinding(ir::Var var) const {
      iassert(var.defined());
