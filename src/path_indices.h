#ifndef _JAYL_PATH_INDICES_H
#define _JAYL_PATH_INDICES_H
...
/// A SetEndpointPathIndex uses a Set's endpoint list to find path neighbors.
    class SetEndpointPathIndex : public PathIndexImpl {
      ...
      private:
      const jayl::Set &edgeSet;

      void print(std::ostream &os) const;

      friend PathIndexBuilder;

      SetEndpointPathIndex(const jayl::Set &edgeSet);
    };
...
/// recursively constructed from path expressions).
    class PathIndexBuilder {
      ...
      PathIndexBuilder(std::map<std::string, const jayl::Set *> bindings)
        : bindings(bindings) {}
      ...
      void bind(std::string name, const jayl::Set *set);

      const jayl::Set *getBinding(pe::Set pset) const;

      const jayl::Set *getBinding(ir::Var var) const;

      private:
      std::map<std::pair<PathExpression, unsigned>, PathIndex> pathIndices;
      std::map<std::string, const jayl::Set *> bindings;
    };
