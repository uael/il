...
void storeTime(int i, double value) {
  jayl::ir::TimerStorage::getInstance().storeTime(i, value);
}
...
/// Temporary external spmm implementation until Jayl supports assembling
...
#define SOLVER_ERROR                                            \
do {                                                            \
  ierror << "Solvers require that Jayl was built with Eigen."; \
} while (false)
