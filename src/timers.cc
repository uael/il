...
namespace jayl {
  namespace ir {
    ...
    void printTimes() {
      ...
      for (auto line: jayl::ir::TimerStorage::getInstance().getSourceLines()) {
        ...
        int index = jayl::ir::TimerStorage::getInstance().getTimedLineIndex(test);
        if (index >= 0) {
      ...
      printf("Total Time: %f (seconds)\n",
        jayl::ir::TimerStorage::getInstance().getTotalTime() / 1000000.0);
    }
...
// Singleton
    class InsertTimers : public IRRewriter {
      ...
      private:
      int counter = 0;
      Var timeStartVar = Var(INTERNAL_PREFIX("jayl_internal_time_var"), Float);
